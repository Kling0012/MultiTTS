...
3. サーバー内で `/join` を実行するとボイスチャネルに参加し、
   コマンドを実行したテキストチャネルのメッセージを監視します。
   そのチャンネルに投稿された内容が中国語と判定された場合、自動で読み上げます。
   `/leave` で停止し、`/say` コマンドでは任意のテキストを読み上げられます。
4. 音声再生には `ffmpeg` が必要です。実行可能ファイルが `PATH` にあることを確認してください。
